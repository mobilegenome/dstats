...
N<-length(ind)
cat(file)
# if (grepl("abbabababbaa",file))
# {
#   if(N*(N-1)*(N-2)+((N*(N-1)*(N-2))/2)!=ncol(ab)-3){
...
Ncomp<-ncol(ab)-3 # *(2/3)
cat("H1","H2","H3","nABBA","nBABA","nBBAA",c("Dstat","jackEst","SE","Z"), '\n', file=paste(outfile,".txt",sep=""), append=FALSE ,sep="\t")
...
for(h3 in 1:N){
  for(h2 in 1:N){
    ...
    for(h1 in 1:N){
      ...
      if(h1>=h2)
        next
      dat <- ab[,comp*3+3+1:2] #       dat<-ab[,comp*2+3+1:2]
      bbaa <- ab[,comp*3+6]
      keep<-rowSums(dat)>0
      ...
      cat(ind[h1],ind[h2],ind[h3],colSums(dat),sum(bbaa),c(est,jack,sd,Z), '\n', file=paste(outfile,".txt",sep=""), append=TRUE ,sep="\t")
